...
1. First, a model goes into production, and the **serving pipeline** starts delivering predictions.
   ...
   |                    | Static/Offline/Batch inference | Dynamic/Online inference |
   |--------------------|-----------------|------------------|
   | **Definition** | model makes predictions on a bunch of common unlabeled examples and then caches those predictions somewhere | model only makes predictions on demand, for example, when a client requests a prediction. |
   | **Advantages**     | No worry on cost of inference. Can do **post-verification** of predictions before pushing | Can infer a prediction on any new item as it comes in, which is great for **long tail (less common) predictions**. |
   | **Disadvantages**  | Only serve cached predictions, may **miss predictions for uncommon input** examples; Update latency is likely measured in hours or days | **Compute intensive and latency sensitive**. This combination may **limit model complexity**. Monitoring needs are more intensive |
...
3. Based on a schedule or a trigger, the **training and validation pipelines** train and validate a new model using the datasets generated by the data pipeline.
   ...
   |                    | Static/Offline training | Dynamic/Online training |
   |--------------------|-----------------|------------------|
   | **Definition** | train a model once and then serve it for a while. | train a model continuously or at least frequently. serve the most recently trained model. |
...
- **Integration tests** ensure that different components of the ML pipeline work together seamlessly, and should be run continuously and for new model/software versions.
- Before serving a new model, validate its quality by checking for sudden and slow degradations against previous versions and fixed thresholds.
...
- Write a data schema to validate raw data
  ...
  To monitor your data, you should continuously check it against expected statistical values by writing rules that the data must satisfy. This collection of rules is called a data schema. Catch data errors such as:
...
- Write unit tests to validate feature engineering
  ...
  Because feature engineered data can be very different from raw input data, you must check feature engineered data separately from your checks on raw input data. Unit tests should check for conditions such as the following:
...
- Check metrics for important data slices
  ...
  A model with great overall metrics might still make terrible predictions for certain situations. Identify data slices of interest. Then compare model metrics for these data slices against the metrics for your entire dataset.

- Track real-world metrics

  Model metrics don't necessarily measure the real-world impact of your model. For example, changing a hyperparameter might increase a model's AUC, but how did the change affect user experience?

- Check for training-serving skew

  Training-serving skew means your input data during training differs from your input data in serving. Always consider what data is available to your model at prediction time. During training, use only the features that you'll have available when serving.
